...
export class VideoController {
    ...
    async createVideo(@Res() response, @Req() request, @Body() video: Video, @UploadedFiles() files: { video?: Express.Multer.File[], cover?: Express.Multer.File[] }) {
        const fvf = files.video?files.video[0].filename:"";
        const fcf = files.cover?files.cover[0].filename:"";
        const requestBody = { createdBy: request.user, title: video.title, video: fvf, coverImage: fcf }
